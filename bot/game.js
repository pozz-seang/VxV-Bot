...
const { avatar } = require("./functions");
...
const gameFlip = async (msg, req) => {
    ...
    if (req[1]) {
        if (Number(req[1]) == NaN) return msg.channel.send(`🚫** | ${getName(msg)}**, Invalid arguments!!`)
    ...
    const msgRef = await msg.channel.send(`**${getName(msg)}** spent 💵 **${cf.money}** and chose **${cf.bit}**\n The coin spins... 🪙`)
    ...
    setTimeout(() => {
        msgRef.edit(`**${getName(msg)}** spent 💵 **${cf.money}** and chose **${cf.bit}**\n The coin spins... 🪙 and you ${cf.msg}`)
    }, 1000);
...
async function gFlip(msg, req) {
    ...
    if (talkedRecently.has(msg.author.id)) {
        ...
        var TH = req[2]
        if (TH == "h" || TH == "H" || TH == "head" || TH == "Head" || TH == "heads" || TH == "Heads" || TH == "HEADS") {
            TH = "HEADS"
        } else if (TH == "t" || TH == "T" || TH == "tail" || TH == "Tail" || TH == "tails" || TH == "Tails" || TH == "TAILS") {
            TH = "TAILS"
        } else {
            sendEB(msg, "Please select heads or tails.")
            return
        ...
        sendEB(msg, results + "! Your new balance is: " + new Intl.NumberFormat().format(money))
